...
import { getParticipantCompaniesByEvent, addParticipantCompany, updateParticipantCompany, deleteParticipantCompany, getButtonConfigs } from '../../services/api';
...
const emptyCompany: Omit<ParticipantCompany, 'id'> = {
  name: '', boothCode: '', buttonIds: [], responsible: '', contact: '', responsiblePhone: '', eventId: ''
};
...
const ParticipantCompaniesManager: React.FC<Props> = ({ eventId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isEditing) {
      await updateParticipantCompany(currentCompany as ParticipantCompany);
    } else {
      await addParticipantCompany(currentCompany as Omit<ParticipantCompany, 'id'>);
    }
    fetchData();
    handleCloseModal();
  };
          ...
          return (
            <div key={company.id} className="bg-card p-5 rounded-lg shadow-md flex flex-col justify-between">
              <div className="flex-grow">
                ...
                <p className="text-sm text-text-secondary mb-2">Cód. Estande: {company.boothCode}</p>
